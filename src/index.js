...
// The server process
const JAR = 'Girvi-0.0.1-SNAPSHOT.war'; // how to avoid manual update of this?
...
function startServer(port) {
  ...
  const server = `${path.join(app.getAppPath(),'src', JAR)}`;
...
function loadHomePage(baseUrl) {
  ...
  setTimeout(function cycle() {
    axios.get(baseUrl).then(response => {
      ...
      logger.info(`Why : ${error}`);
      if (checkCount < MAX_CHECK_COUNT) {
        checkCount++;
        setTimeout(cycle, 1000);
      } else {
        dialog.showErrorBox('Server timeout',
          `App does not receive server response.`);
        app.quit()
      }
    });
